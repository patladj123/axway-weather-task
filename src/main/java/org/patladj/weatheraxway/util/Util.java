package org.patladj.weatheraxway.util;
...
//        System.out.println("roundToPrec float="+v);
    ...
    public static final float fahrenheitToCelsiusRoundedToPrecisionOne(float fah) {
        float ft = 5.0f / 9.0f;
        float cel = ft * (fah - 32);
        return roundToPrec(cel, 1);
    }

    public static final float fahrenheitToCelsiusRoundedToPrecisionOne_FromString(String fahStr) {
        float fah = Float.parseFloat(fahStr);
        float ft = 5.0f / 9.0f;
        float cel = ft * (fah - 32.0f);
        return roundToPrec(cel, 1);
    }

    public static final float kelvinToCelsiusRoundedToPrecisionOne_FromString(String kelStr) {
        float kel = Float.parseFloat(kelStr);
        float celsius = kel - 273.15f;
        return roundToPrec(celsius, 1);
    }
}
